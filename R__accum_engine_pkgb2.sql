CREATE OR REPLACE PACKAGE BODY accum_engine AS
    ...
    PROCEDURE run_engine IS
        ...
        select stage_2_id,claim_type,claim_amount,member_id
        from stage_2;
--        SELECT DISTINCT
--            a.member_id,
--            nvl((
...
--            stage_2 a;
    ...
    --Check claims to see if new member accumulation records need to be created  
        ...
        LOOP
            ...
            IF vclaim_type = 'RX' THEN
                accumulate_rx_claims(vstage2_claim_amount_rx, vstage2_member_record_id);
            END IF;
            IF vclaim_type = 'MED' THEN
                accumulate_med_claims(vstage2_claim_amount_rx, vstage2_member_record_id);
            END IF;
